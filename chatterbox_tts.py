...
# Use this to add AWS S3 tokens (already created):
...
S3_BUCKET_NAME = "ai-voice-saaas"  # replace with your actual S3_BUCKET_NAME value
S3_MOUNT_PATH = "/s3"
s3_bucket = modal.CloudBucketMount(
    S3_BUCKET_NAME,
    secret=modal.Secret.from_name("aws-s3"),
    read_only=True,
)
...
class Chatterbox:
    ...
    def serve(self):
        ...
        def generate_speech(request: TTSRequest):
            voice_path = Path(S3_MOUNT_PATH) / request.voice_key
...
def test(
    ...
    audio_prompt_path = f"{S3_MOUNT_PATH}/{voice_key}"
